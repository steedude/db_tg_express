...
bot.command('start', (ctx) => {
  return ctx.reply("請選擇功能或直接輸入'上班'",
...
const registerWizard = new WizardScene(
  ...
  async ctx =>  {
    ...
    try {
      ...
      console.log(result)
      ctx.reply(`已成功註冊，name: ${ctx.wizard.state.data.name}，passward:${ctx.wizard.state.data.passward}`)
    }catch(e){
      ctx.reply(e);
    } finally {
      return ctx.scene.leave();
...
const reserveWizard = new WizardScene(
  ...
  ctx => {
    ...
    ctx.reply('請輸入日期', Markup
    .keyboard([
      ['01號', '02號', '03號', '04號','05號', '06號', '07號', '08號','09號', '10號'],
      ['11號', '12號', '13號', '14號','15號', '16號', '17號', '18號','19號', '20號'],
      ['21號', '22號', '23號', '24號','25號', '26號', '27號', '28號','29號', '30號','31號'],
    ])
    .oneTime()
    .resize()
  );
  ...
  async ctx =>  {
    ...
    ctx.reply(`${datas.month.slice(0,2)}/${datas.date.slice(0,2)} ${datas.hour}:${datas.mins}`)
  }
);
